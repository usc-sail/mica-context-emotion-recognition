...
def main(config_data):
    ...
    train_pds=CAER_Face_only_dataset(
            csv_file=train_csv_file,
            label_map_file=label_map_file,
            transforms=transforms_img,
            bbox_file=bbox_file)

    val_pds=CAER_Face_only_dataset(
    ...
    model=CAER_face_only_model(**model_dict)
    ...
    for epoch in range(1, max_epochs+1): #main outer loop
            ...
            valid_accuracy,f1_score_val,precision_score_val,recall_score_val,val_loss=gen_validate_score_face_only_model(val_dl,model,device,discrete_criterion)
