...
train_test_val_config_file="/home/dbose_usc_edu/codes/context-emotion-recognition/configs/CAER_configs/config_caption_person_MHA_dual_model_scene_AdamW_train_test_val_multiple_seeds_AdamW.yaml"
...
caption_file=config_data['data']['caption_file']
print(caption_file)
scene_folder=config_data['data']['scene_folder']
...
for seed_value in seed_value_list:
    ...
    train_pds=CAER_Face_Caption_Scene_Feature_AVD_Discrete_dataset(
            tokenizer=tokenizer,
    ...
    val_pds=CAER_Face_Caption_Scene_Feature_AVD_Discrete_dataset(
            tokenizer=tokenizer,
    ...
    train_dl=DataLoader(train_pds,batch_size=batch_size,shuffle=shuffle_train,num_workers=4)
    val_dl=DataLoader(val_pds,batch_size=batch_size,shuffle=shuffle_val,num_workers=4)
    ...
    model_dict={
            'dim_model':config_data['model']['dim_model'],
            ...
            'add_bias':config_data['model']['add_bias'],
            'activation':'relu',
            'batch_first':config_data['model']['batch_first'],
            'num_layers':config_data['model']['num_layers'],
            'scene_feat_dim':config_data['model']['scene_feat_dim'],
            'face_model_option':feature_model,
            'face_max_len':config_data['model']['person_max_len'],
            'text_max_len':config_data['model']['text_max_len'],
            'scene_max_len':config_data['model']['scene_max_len'],
            ...
            'add_pos':config_data['model']['add_pos']
    }
    
    model=caption_face_dual_MHA_model_scene_late_fusion_AVD_Discrete(**model_dict)
    ...
    if(config_data['loss']['discrete_loss_option']=='cross_entropy_loss'):
        discrete_criterion=cross_entropy_loss(device=device)
        print(discrete_criterion)
    
    if(config_data['optimizer']['scheduler']=='linear_scheduler'):
        scheduler=linear_schedule_with_warmup(optim_example,num_warmup_steps=int(config_data['optimizer']['warmup_steps']),
                                num_training_steps=max_epochs*len(train_dl))
    ...
    for epoch in range(1, max_epochs+1): #main outer loop
            ...
            valid_accuracy,f1_score_val,precision_score_val,recall_score_val,val_loss=gen_validate_score_face_model_with_text_scene_MCAN(val_dl,model,device,discrete_criterion) #val script for MCAN should be same as MHA
            logger.info('Validation loss:{:.3f}, Validation accuracy:{:.3f},Validation f1:{:.3f}'.format(val_loss,valid_accuracy,f1_score_val))
            model.train(True)
    ...
    model.eval()
    val_f1_multiple_seeds_list.append(f1_score_val)
    ...
    print('Current seed: %d, Test loss: %f, Test accuracy: %f, Test f1: %f' %(seed_value,test_loss,test_accuracy,f1_score_test))

    test_f1_multiple_seeds_list.append(f1_score_test)
    test_acc_multiple_seeds_list.append(test_accuracy)
...
df=pd.DataFrame({'Random_seeds':seed_value_list,
            ...
            'Test_acc': test_acc_multiple_seeds_list,
            'Test_f1':test_f1_multiple_seeds_list,
            'Test_loss':test_loss_multiple_seeds_list,
            'Val_f1':val_f1_multiple_seeds_list})

df.to_csv(os.path.join(model_loc_subfolder,'Test_multiple_seeds_statistics_OFA_captions.csv'))
